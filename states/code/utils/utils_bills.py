...
def get_data_list_of_dicts( data_path=data_path, congress_files=congress_files ):
...
def get_ngram_inverted_index( bill_list, ngram_dict ):
    ngram_inverted_index = collections.defaultdict( list )
    count = 0
    
    for idx, bill in enumerate( bill_list ):
        if idx % 10000 == 0:
            sys.stderr.write( "%s " % idx )
        for ngram in list( ngram_dict[bill] ):
            ngram_inverted_index[ngram].append( idx )
